...
public class UserController {
    ...
    /**
     * 닉네임 중복 체크 API (특정 사용자 제외)
     * 닉네임 변경 시 자신을 제외하고 중복을 확인합니다.
     * 
     * @param request 확인할 닉네임과 사용자 ID
     * @return 200 OK: 중복 여부 (exists: boolean)
     */
    @PostMapping("/check-nickname-exclude")
    public ResponseEntity<Map<String, Boolean>> checkNicknameExcludeUser(@RequestBody Map<String, Object> request) {
        String nickname = (String) request.get("nickname");
        Long userId = Long.valueOf(request.get("userId").toString());
        boolean exists = userService.isNicknameExistsExcludeUser(nickname, userId);
    ...
    /**
     * 마이페이지 사용자 권한 변경 API
     * 로그인한 사용자가 자신의 권한을 변경할 수 있습니다.
     * (일반유저 ↔ 회사원 ↔ 관리자)
     * 
     * @param userId 권한을 변경할 사용자 ID (본인만 가능)
     * @param roleRequest 새로운 권한 정보
     * @return 200 OK: 수정된 사용자 정보
    ...
    public ResponseEntity<?> changeMyRole(@PathVariable Long userId, 
                                        @RequestBody Map<String, String> roleRequest) {
        try {
            String roleStr = roleRequest.get("role");
            if (roleStr == null) {
                Map<String, String> error = new HashMap<>();
                error.put("error", "권한 정보가 필요합니다.");
                return ResponseEntity.badRequest().body(error);
            }
            
            UserRole newRole = UserRole.valueOf(roleStr.toUpperCase());
            UserResponse updatedUser = userService.changeUserRole(userId, newRole);
            ...
            Map<String, String> error = new HashMap<>();
            error.put("error", "유효하지 않은 권한입니다. (GENERAL, COMPANY, ADMIN 중 선택)");
            return ResponseEntity.badRequest().body(error);
        } catch (RuntimeException e) {
            Map<String, String> error = new HashMap<>();
            error.put("error", e.getMessage());
    ...
    /**
     * 마이페이지 닉네임 변경 API
     * 로그인한 사용자가 자신의 닉네임을 변경할 수 있습니다.
     * 
     * @param userId 닉네임을 변경할 사용자 ID (본인만 가능)
     * @param nicknameRequest 새로운 닉네임 정보
     * @return 200 OK: 수정된 사용자 정보
     *         400 Bad Request: 잘못된 요청 또는 중복된 닉네임
     *         404 Not Found: 사용자를 찾을 수 없음
     */
    @PutMapping("/{userId}/change-nickname")
    public ResponseEntity<?> changeMyNickname(@PathVariable Long userId, 
                                            @RequestBody Map<String, String> nicknameRequest) {
        try {
            String newNickname = nicknameRequest.get("nickname");
            if (newNickname == null || newNickname.trim().isEmpty()) {
                Map<String, String> error = new HashMap<>();
                error.put("error", "닉네임을 입력해주세요.");
                return ResponseEntity.badRequest().body(error);
            }
            
            if (newNickname.length() < 2) {
                Map<String, String> error = new HashMap<>();
                error.put("error", "닉네임은 2자 이상이어야 합니다.");
                return ResponseEntity.badRequest().body(error);
            }
            
            UserResponse updatedUser = userService.changeUserNickname(userId, newNickname.trim());
    ...
    public ResponseEntity<?> getMyProfile(@PathVariable Long userId) {
        ...
        if (user.isPresent()) {
            ...
            roleDescriptions.put("GENERAL", "일반 사용자 - 자유게시판, Q&A 게시판 이용 가능");
            roleDescriptions.put("COMPANY", "기업 사용자 - 기업게시판 작성 및 모든 게시판 이용 가능");
            roleDescriptions.put("ADMIN", "관리자 - 모든 게시판 관리 및 사용자 관리 권한");
            
            response.put("roleDescriptions", roleDescriptions);
            response.put("availableRoles", UserRole.values());
